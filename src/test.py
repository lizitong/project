...
def parse(line):
	...
	for sr in srs:
		...
		if point.within(polygon):
			county = sr.record[6]
		"""
		if point_inside_polygon(x,y,coords):
			county = sr.record[6]
	if county == "Not found":
		county = "Richmond"
		#print "********************"
...
def main(sc):
	"""
	trips = sc.textFile("/taxidata/*/")
	mappedTrips = trips.map(lambda line:line.split(",")).filter(deleteInvalidLines)
	# print mappedTrips.collect()
	newTrips = mappedTrips.map(parse)
	#print newTrips.collect()
	testTrips = mappedTrips.reduceByKey(add)
	print testTrips.collect()
	sortedTrips = testTrips.sortBy(lambda x:x[1], False)
